...
for(const p of paths)if(/teacher|private|\.map$/i.test(p))throw Error('Приватный путь в сборке: '+p);
const text=paths.filter(p=>/\.(html|js|json)$/.test(p)).map(p=>fs.readFileSync(p,'utf8')).join('\n');
...
console.log(JSON.stringify({files:paths.length,privateScriptsChecked:checked,assessment:'public autonomous educational keys',status:'passed'}));
